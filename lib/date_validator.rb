class DateValidator
  ...
  def build_date
    date = Date.new(@split_year.to_i, parse_month(@split_month).to_i, @split_day.to_i)
    date if date.day == @split_day.to_i && date.month == parse_month(@split_month).to_i && date.year == @split_year.to_i
  rescue
    false
  end
  ...
  def all_partials_empty?
    begin
      { d: @split_day.empty?, m: @split_month.empty?, y: @split_year.empty? }.values.all?
    rescue
      false
    end
  end
